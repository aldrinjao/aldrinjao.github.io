...
var AppComponent = /** @class */ (function () {
    ...
    AppComponent.prototype.filterSelection = function () {
        ...
        var omRange = this.SOIL_DATA.filter(function (f) {
            return (f.omValue >= _this.omLowValue && f.omValue <= _this.omHighValue);
        });
        var nRange = this.SOIL_DATA.filter(function (f) {
            return (f.nValue >= _this.nLowValue && f.nValue <= _this.nHighValue);
        });
        var pRange = this.SOIL_DATA.filter(function (f) {
            return (f.pValue >= _this.pLowValue && f.pValue <= _this.pHighValue);
        });
        var kRange = this.SOIL_DATA.filter(function (f) {
            return (f.kValue >= _this.kLowValue && f.kValue <= _this.kHighValue);
        });
        // filter array for numbers in a range
